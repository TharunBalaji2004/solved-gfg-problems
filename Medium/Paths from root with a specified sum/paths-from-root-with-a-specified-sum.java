...
class Solution {
    private static void helper(ArrayList<ArrayList<Integer>> res, ArrayList<Integer> path, Node root, int curr, int target) {
        if (root == null) return;
        ...
        helper(res, path, root.left, curr, target);
        helper(res, path, root.right, curr, target);
        
        path.remove(path.size()-1);
    }
    
    public static ArrayList<ArrayList<Integer>> printPaths(Node root, int target) {
